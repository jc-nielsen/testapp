...
var PromiseCancelable = function () {
  ...
  _createClass(PromiseCancelable, [{
    key: '_createPromise',
    value: function _createPromise(basePromise) {
      var _this = this;
...
(function () {
   ...
   */
  function numberFormat(num, formatString) {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      var _mask = parts[i].match(maskPattern);

      var colorPresent = _color && _color[1].length;

      switch (true) {
        case i === 0:
          colors.positive = colorPresent ? _color[1] : options.colors.positive;
          colors.negative = options.colors.negative;
          colors.neutral = options.colors.neutral;

          masks.positive = _mask[2];
          masks.negative = _mask[2];
          masks.neutral = _mask[2];
          break;
        case i === 1:
          colors.negative = colorPresent ? _color[1] : options.colors.negative;
          masks.negative = _mask[2];
          break;
        case i === 2:
          colors.neutral = colorPresent ? _color[1] : options.colors.neutral;
          masks.neutral = _mask[2];
          break;
      }
    }
    ...
    switch (true) {
      case Number.isNumber(num):
        text = SSF.format(num > 0 ? masks.positive : num < 0 ? masks.negative : masks.neutral, Number(num));
        color = num > 0 ? colors.positive : num < 0 ? colors.negative : colors.neutral;

        if (currency && currency.length > 0) {
          text = Number(text).toLocaleString(options.locale, {
            style: 'currency',
            currency: currency
          });
        }
  ...
  var NumberFormatter = function () {
     ...
     */
    function NumberFormatter(num, type, formatString) {
      var options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};
    ...
    _createClass(NumberFormatter, [{
      ...
      value: function __format() {
        ...
        for (var i = 0; i < parts.length; i++) {
          ...
          var _color2 = parts[i].match(colorPattern);
          var _mask2 = parts[i].match(maskPattern);

          var colorPresent = _color2 && _color2[1].length;

          switch (true) {
            case i === 0:
              colors.positive = colorPresent ? _color2[1] : '#26B910';
              colors.negative = '#E00020';
              colors.neutral = '#151921';

              masks.positive = _mask2[2];
              masks.negative = _mask2[2];
              masks.neutral = _mask2[2];
              break;
            case i === 1:
              colors.negative = colorPresent ? _color2[1] : '#E00020';
              masks.negative = _mask2[2];
              break;
            case i === 2:
              colors.neutral = colorPresent ? _color2[1] : '#151921';
              masks.neutral = _mask2[2];
              break;
          }
        }
      ...
      value: function __currency() {
        switch (this.type) {
          case '$':
            {
              ...
              if (localizedStr.indexOf('$') === 0) {
                this.result.currencyPos = 'start';
                break;
              }

              this.result.currencyPos = 'end';
              break;
            }

          case '€':
            {
              var _localizedStr = Number(this.result.num).toLocaleString(this.options.locale, {
                style: 'currency',
                currency: 'EUR'
              });

              if (_localizedStr.indexOf('€') === 0) {
                this.result.currencyPos = 'start';
                break;
              }
          ...
          case '£':
            {
              var _localizedStr2 = Number(this.result.num).toLocaleString(this.options.locale, {
                style: 'currency',
                currency: 'GBP'
              });

              if (_localizedStr2.indexOf('€') === 0) {
                this.result.currencyPos = 'start';
                break;
              }
          ...
          case '%':
            {
              ...
              this.result.num = Number(this.num).toLocaleString(this.options.locale, {
                style: 'percent',
        ...
        // switch (true) {
        //   case this.formatString.indexOf('$') >= 0: {
        //     const localizedStr = this.result.num.toLocaleString('ru-RU', {style: 'currency', currency: 'USD'});
        //
        //     //TODO: Regexp instead of indexOf
        //     if (localizedStr.indexOf('$') === 0) {
        //       this.result.currencyPos = 'start';
        //       this.result.currency = localizedStr.charAt(0);
        //       break;
        //     }
        //
        //     this.result.currency = localizedStr.charAt(localizedStr.length - 1);
        //     this.result.currencyPos = 'end';
        //     break;
        //   }
        //   case this.formatString.indexOf('%') >= 0: {
        //     this.result.num = this.num.toLocaleString('en-US', {style: 'percent'});
        //
      ...
      value: function perform() {
        ...
        return {
          text: '' + this.result.sign + (this.result.currencyPos == 'start' ? this.type : '') + this.result.num + this.result.abbr + (this.result.currencyPos == 'end' ? this.type : ''),
  ...
  function numToFormattedString(num) {
    switch (true) {
      case mask.indexOf('$') >= 0:
        {
          if (num >= 10000) {
            var abbrNum = abbreviateNum(num);
            var roundedNum = abbrNum.slice(0, -1);

            var abbr = abbrNum.slice(abbrNum.length - 1);
            var formattedAbbrNum = Number(roundedNum).toLocaleString(locale, { style: 'currency', currency: currency });

            return '' + formattedAbbrNum + abbr;
          }
...
nv.ModelExtension = function (_Factory) {
  ...
  _createClass(_class, [{
    ...
    value: function _useModules() {
      ...
      // }
      this.modulesPromise = PromiseCancelable.resolve();
      this._forEachEnabledModule(function (module) {
        _this8.modulesPromise = _this8.modulesPromise.then(function () {
          return module._process();
        });
      });
